...
def crypto_price_message_handler(message):
    ...
    try:
        if price_response:
            ...
            bot.send_message(chat_id=chat_id, text=f"Price of {crypto_id} CryptoCurrency : {price}$ USD")
            if crypto_id == 'Bitcoin':
                ...
                bot.send_message(chat_id=chat_id, text=f"Price of bitcoin in forex currrencies:\n{bitc_EUR}$ EUR\n{bitc_INR}$ INR\n{bitc_MXN}$ MXN\n{bitc_CZK}$ CZK\n{bitc_IDR}$ IDR\n{bitc_BGN}$ BGN\n{bitc_ILS}$ ILS\n{bitc_GBP}$ GBP\n{bitc_HKD}$ HKD\n{bitc_AUD}$ AUD\n{bitc_CHF}$ CHF\n{bitc_KRW}$ KRW\n{bitc_CNY}$ CNY\n{bitc_SGD}$ SGD\n{bitc_SEK}$ SEK\n{bitc_MYR}$ MYR\n{bitc_RON}$ RON\n{bitc_HUF}$ HUF\n{bitc_JPY}$ JPY\n{bitc_CAD}$ CAD\n{bitc_DKK}$ DKK\n{bitc_TRY}$ TRY\n{bitc_HRK}$ HRK\n{bitc_NZD}$ NZD\n{bitc_THB}$ THB\n{bitc_BRL}$ BRL\n{bitc_PLN}$ PLN\n{bitc_PHP}$ PHP\n{bitc_ZAR}$ ZAR\n\nFor List of Supported Currency codes just click on /help\nFor todays forex prices just click on /forex\nلطفا از حروف کوچک استفاده کنید بات از کلماتی که اولشان با حروف بزرگ شروع میشود را تشخیص نمیدهد")
        else:
            if crypto_id == '/start':
                bot.send_message(chat_id=chat_id, text='Welcome to CryptoForexBot ask me any CryptoCurrency name like bitcoin or ethereum\n\nFor todays forex prices just click on /forex\nFor List of Supported Currency codes click on /help\n\nNote: Please use lowercase\nلطفا از حروف کوچک استفاده کنید')
            elif crypto_id == '/help':
        ...
        if KeyError():
            bot.send_message(chat_id=chat_id, text="Please use lowercase for example: bitcoin")
